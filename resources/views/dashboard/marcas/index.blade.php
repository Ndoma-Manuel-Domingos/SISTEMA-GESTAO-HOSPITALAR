...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('messages.marcas') }}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                                        ...
                                        <td class="text-right">
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu" role="menu">
                                                    ...
                                                    <a class="dropdown-item" href="{{ route('marcas.show', $item->id) }}"><i class="fas fa-eye text-light-primary"></i> {{ __('messages.mais_detalhes') }} </a>
                                                    @endif
                                                    @if (Auth::user()->can('editar todos') || Auth::user()->can('editar marca'))
                                                    ...
                                                    @if (Auth::user()->can('eliminar todos') || Auth::user()->can('eliminar marca'))
    ...
    <form action="{{ route('marcas.store') }}" method="post" class="">
        @csrf
        <div class="modal fade" id="modal-lg">
            <div class="modal-dialog modal-lg  modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">{{ __('messages.marcas') }}</h4>
...
<script>
    ...
    $(document).on('click', '.edit-folder', function(e) {
        ...
        $.ajax({
            url: `{{ route('marcas.edit', ':id') }}`.replace(':id', recordId)
            , method: 'GET'
            , data: {
    ...
    $(document).on('click', '.delete-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                // Envia a solicitação AJAX para excluir o registro
                $.ajax({
                    url: `{{ route('marcas.destroy', ':id') }}`.replace(':id', recordId)
                    , method: 'DELETE'
                    , data: {
                        _token: '{{ csrf_token() }}', // Inclui o token CSRF
                    }
