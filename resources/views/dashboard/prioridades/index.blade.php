...
<div class="content-wrapper">
                ...
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">
                                @if (Auth::user()->can('criar todos') || Auth::user()->can('criar prioridade'))
                                        ...
                                        <td class="text-right">
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu" role="menu">
                                                    @if (Auth::user()->can('editar todos') || Auth::user()->can('editar prioridade'))
                                                    <a href="#" data-id="{{ $item->id ?? "" }}" class="dropdown-item edit-folder text-light-success"><i class="fas fa-edit"></i> {{ __('messages.actualizar') }}</a>
                                                    @endif
                                                    @if (Auth::user()->can('eliminar todos') || Auth::user()->can('eliminar prioridade'))
    ...
    <form action="{{ route('prioridades.store') }}" method="post" class="">
        @csrf
        <div class="modal fade" id="modal-lg">
            <div class="modal-dialog modal-lg  modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">Prioridades</h4>
                    ...
                    <div class="modal-body row py-4">
                        ...
                        <div class="col-12 col-md-12">
                            ...
                            <div class="input-group mb-3">
                                <select type="text" class="form-control form-control-lg" id="cor" name="cor">
                                    <option value="#FF4C4C">🔴</option>
                                    <option value="#FF9900">🟠</option>
                                    <option value="#FFCC00">🟡</option>
                                    <option value="#66CC66">🟢</option>
                                    <option value="#4DA6FF">🔵</option>
...
<script>
    ...
    $(document).on('click', '.edit-folder', function(e) {
        ...
        $.ajax({
            url: `{{ route('prioridades.edit', ':id') }}`.replace(':id', recordId)
            , method: 'GET'
            , data: {
    ...
    $(document).on('click', '.delete-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                // Envia a solicitação AJAX para excluir o registro
                $.ajax({
                    url: `{{ route('prioridades.destroy', ':id') }}`.replace(':id', recordId)
                    , method: 'DELETE'
                    , data: {
                        _token: '{{ csrf_token() }}', // Inclui o token CSRF
                    }
