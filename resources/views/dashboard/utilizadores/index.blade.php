            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">{{ __('messages.utilizador') }}</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                ...
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">
                                <a href="{{ route('utilizadores.create') }}" class="btn btn-light-primary"> <i class="fas fa-plus"></i> {{ __('messages.novo') }}</a>
                            </h3>
                        ...
                        <div class="card-body table-responsive">
                            <table class="table table-hover text-nowrap" id="carregar_tabela" style="width: 100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ $util->id }}</td>
                                        <td><a href="{{ route('utilizadores.edit', $util->id) }}">{{ $util->name }}</a>
                                        </td>
                                        <td>{{ $util->email }}</td>
                                        <td>
                                        ...
                                        <td>
                                            @foreach ($util->minhas_lojas as $item)
                                            <span class="badge badge-light-info">{{ $item->loja->nome }}</span>
                                            @endforeach
                                        </td>
                                        ...
                                        <td class="text-right">
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu" role="menu">
                                                    <a class="dropdown-item" href="{{ route('utilizadores.show', $util->id) }}"><i class="fas fa-eye text-light-primary"></i> {{ __('messages.mais_detalhes') }} </a>
                                                    <a class="dropdown-item" href="{{ route('utilizadores.edit', $util->id) }}"><i class="fas fa-edit text-light-success"></i> {{ __('messages.actualizar') }}</a>
                                                    <div class="dropdown-divider"></div>
                                                    <button class="btn btn-light-danger dropdown-item delete-record" data-id="{{ $util->id }}">
    ...
    $(document).on('click', '.delete-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: `{{ route('utilizadores.destroy', ':id') }}`.replace(':id', recordId)
                    , method: 'DELETE'
                    , data: {
