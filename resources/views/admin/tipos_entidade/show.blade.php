                    ...
                    <tr>
                      <td>{{ $item->nome }} <a href="{{ route('eliminar_permissao', $item->id) }}" class="float-right text-light-danger" title="remover permissão ao Perfil"><i class="fas fa-close"></i></a></td>
                        ...
                        <div class="card-footer clearfix d-flex">
                            <a href="{{ route('tipos-entidade.edit', $tipo_entidade->id) }}" class="btn btn-light-success mx-1">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form action="{{ route('tipos-entidade.destroy', $tipo_entidade->id ) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-light-danger mx-1" onclick="return confirm('Tens Certeza que Desejas excluir esta Tipo de entidade?')">
