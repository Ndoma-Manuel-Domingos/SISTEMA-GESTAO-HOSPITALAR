...
<div class="content-wrapper">
                            ...
                            <table class="table table-hover text-nowrap" id="carregar_tabela" style="width: 100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $item->nome ?? "" }}</td>
                                        <td>{{ $item->status }}</td>
                                        <td>{{ $item->irt == "Y" ? __('messages.sim') : __('messages.nao') }}</td>
                                        <td>{{ $item->inss == "Y" ? __('messages.sim') : __('messages.nao') }}</td>
                                        <td>{{ number_format($item->limite_isencao, 2, ',', '.') }}</td>
                                        ...
                                        <td class="text-right">
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu" role="menu">
                                                    ...
                                                    <a class="dropdown-item" href="{{ route('subsidios.show', $item->id) }}"><i class="fas fa-eye text-light-primary"></i> {{ __('messages.mais_detalhes') }} </a>
                                                    @endif
                                                    @if (Auth::user()->can('editar todos') || Auth::user()->can('editar subsidio'))
                                                    ...
                                                    @if (Auth::user()->can('eliminar todos') || Auth::user()->can('eliminar subsidio'))
    ...
    <form action="{{ route('subsidios.store') }}" method="post" class="">
        @csrf
                    ...
                    <div class="modal-body row py-4">
                        <div class="col-12 col-md-12">
                            ...
                            <div class="input-group mb-3">
                                <input type="text" class="form-control" name="nome" id="nome" value="{{ old('nome') }}" placeholder="Informe a Designação">
                            </div>
                        </div>

                        <div class="col-12 col-md-6">
                            <label for="irt" class="form-label">Sujeito IRT</label>
                            <div class="input-group mb-3">
                        ...
                        <div class="col-12 col-md-6">
                            <label for="inss" class="form-label">Sujeito INSS</label>
                            <div class="input-group mb-3">
                        ...
                        <div class="col-12 col-md-12">
                            <label for="limite_isencao" class="form-label">Limite Isenção</label>
                            <div class="input-group mb-3">
                                <input type="text" class="form-control" name="limite_isencao" id="limite_isencao" placeholder="Informe o Valor de Limite Isenção">
                            </div>
                        </div>
...
<script>
    ...
    $(document).on('click', '.edit-folder', function(e) {
        ...
        $.ajax({
            url: `{{ route('subsidios.edit', ':id') }}`.replace(':id', recordId)
            , method: 'GET'
            , data: {
            ...
            , success: function(response) {
                ...
                document.getElementById('irt').value = response.data.irt;
                document.getElementById('inss').value = response.data.inss;
                document.getElementById('limite_isencao').value = response.data.limite_isencao;
    ...
    $(document).on('click', '.delete-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                // Envia a solicitação AJAX para excluir o registro
                $.ajax({
                    url: `{{ route('subsidios.destroy', ':id') }}`.replace(':id', recordId)
                    , method: 'DELETE'
                    , data: {
                        _token: '{{ csrf_token() }}', // Inclui o token CSRF
                    }
