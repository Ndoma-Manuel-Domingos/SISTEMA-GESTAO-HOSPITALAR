...
<div class="content-wrapper">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">
                                ...
                                <a href="{{ route("grupos.create") }}" class="btn btn-light-success"><i class="fas fa-plus"></i> Agrupar Mesas</a>
                                @endif
                            </h3>
                        </div>
                                        ...
                                        <td class="text-right">
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu" role="menu">
                                                    <a href="{{ route("grupos.create") }}" class="dropdown-item text-light-primary"><i class="fas fa-plus"></i> Agrupar Mesas</a>
    ...
    <form action="{{ route('grupos.store') }}" method="post" class="">
        @csrf
...
<script>
    ...
    $(document).on('click', '.edit-folder', function(e) {
        ...
        $.ajax({
            url: `{{ route('grupos.edit', ':id') }}`.replace(':id', recordId)
            , method: 'GET'
            , data: {
    ...
    $(document).on('click', '.delete-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                // Envia a solicitação AJAX para excluir o registro
                $.ajax({
                    url: `{{ route('grupos.destroy', ':id') }}`.replace(':id', recordId)
                    , method: 'DELETE'
                    , data: {
                        _token: '{{ csrf_token() }}', // Inclui o token CSRF
                    }
