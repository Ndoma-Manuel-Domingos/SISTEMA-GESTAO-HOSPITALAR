        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12 col-md-12 bg-light">
                    <div class="card">
                        <form action="{{ route('reservas.index') }}" method="get" class="mt-3">
                            @csrf
                            <div class="card-body row">
                                ...
                                <div class="col-12 col-md-2 mb-3">
                                    ...
                                    <select type="text" class="form-control select2" name="status_reserva">
                                        <option value="">{{ __('messages.todos') }} </option>
                                        <option value="PENDENTE" {{ $requests['status_reserva'] == 'PENDENTE' ? 'selected' : '' }}>Reservas Pendentes</option>
                                        <option value="EM USO" {{ $requests['status_reserva'] == 'EM USO' ? 'selected' : '' }}>Reservas com Check-In</option>
                                        <option value="SUCESSO" {{ $requests['status_reserva'] == 'SUCESSO' ? 'selected' : '' }}>Reservas com Check-Out</option>
                                        <option value="CANCELADO" {{ $requests['status_reserva'] == 'CANCELADO' ? 'selected' : '' }}>Reservas com cancelados</option>
                                    </select>
                                </div>
                            ...
                            <table class="table table-hover text-nowrap" id="carregar_tabela" style="width: 100%">
                                ...
                                <tbody>
                                    ...
                                    <tr style="background-color: {{ $item->status == 'CANCELADO' ? 'rgba(138, 39, 39, .3)' : '' }}">
                                        ...
                                        <td><a href="{{ route('clientes.show', $item->cliente->id) }}">{{ $item->cliente->nome }}</a>
                                        </td>
                ...
                $.ajax({
                    ...
                    , success: function(response) {
                        ...
                        if (response.status == 401) {
                            Swal.fire({
                                title: 'Erro!'
                                , text: "Por favor, tens contas pendente a pagar então não podemos fazer o check out, podes finalizar as conta agora!"
                                , icon: 'error'
                                , showCancelButton: true
                                , confirmButtonColor: '#198754'
                                , cancelButtonColor: '#dc3545'
                                , confirmButtonText: 'Sim, finalizar a conta!'
                                , cancelButtonText: 'Lembra depois'
                            , }).then((result) => {
                                if (result.isConfirmed) {
                                    // Redirecionar
