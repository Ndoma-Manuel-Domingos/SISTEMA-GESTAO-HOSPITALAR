                        ...
                        <div class="card-body table-responsive">
                            <table class="table table-hover text-nowrap" id="carregar_tabela" style="width: 100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ $item->aluno->id }}</td>
                                        <td><a href="{{ route('clientes.show', $item->aluno->id) }}">{{ $item->aluno->nome }}</a></td>
                                        ...
                                        <td class="text-right">
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu" role="menu">
                                                    <a class="dropdown-item" href="{{ route('clientes.show', $item->aluno->id) }}"><i class="fas fa-eye text-light-primary"></i> {{ __('messages.mais_detalhes') }} </a>
                                                    <a class="dropdown-item" href="{{ route('alunos-matriculas-editar', $item->id) }}"><i class="fas fa-edit text-light-success"></i> {{ __('messages.actualizar') }}</a>

                                                    <div class="dropdown-divider"></div>
                                                    <button type="submit" class="btn btn-light-danger dropdown-item delete-record" data-id="{{ $item->id ?? "" }}">
    ...
    $(document).on('click', '.delete-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: `{{ route('alunos-matriculas-excluir', ':id') }}`.replace(':id'
                        , recordId)
                    , method: 'DELETE'
                    , data: {
