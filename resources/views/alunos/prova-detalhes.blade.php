            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('alunos-provas') }}">{{ __('messages.voltar') }}</a></li>
                                    ...
                                    <table class="table text-nowrap">
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right">
                                                    <select class="form-control opcao @error('opcao') is-invalid @enderror" name="opcao" style="width: 120px;float: right" id="opcao" data-id="{{ $item->id ?? "" }}">
...
<script>
    $(document).on('change', '.opcao', function() {
        let status = $(this).val(); // Valor do Select
        let id = $(this).data('id'); // ID do Estudante

        $.ajax({
            url: `{{ route('alunos-provas-resposta') }}`, // URL da rota
            method: 'POST'
            , data: {
