...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('messages.tarefarios')}}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('messages.voltar') }}</a></li>
                        <li class="breadcrumb-item active">{{ __('messages.tarefarios')}}</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                ...
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">
                                @if (Auth::user()->can('criar todos') || Auth::user()->can('criar tarefario'))
                                <a href="{{ route('tarefarios.create') }}" class="btn btn-light-primary"> <i class="fas fa-plus"></i> {{ __('messages.novo') }}</a>
                                @endif
                            </h3>
                        ...
                        <div class="card-body table-responsive">
                            <table class="table table-hover text-nowrap" id="carregar_tabela" style="width: 100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ $item->id ?? "" }}</td>
                                        <td><a href="{{ route('tarefarios.show', $item->id) }}">{{ $item->nome }}</a>
                                        </td>
                                        <td>{{ number_format($item->preco ?? 0, 2, ',', '.') }}</td>
                                        <td>{{ number_format($item->preco_venda ?? 0, 2, ',', '.') }}</td>
                                        ...
                                        <td class="text-right">
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu" role="menu">
                                                    @if (Auth::user()->can('criar todos') || Auth::user()->can('criar tarefario'))
                                                    <a class="dropdown-item" href="{{ route('tarefarios.associar_tarefario', $item->id) }}"><i class="fas fa-unlink text-light-primary"></i>
                                                        Associar</a>
                                                    @endif
                                                    @if (Auth::user()->can('listar todos') || Auth::user()->can('listar tarefario'))
                                                    <a class="dropdown-item" href="{{ route('tarefarios.show', $item->id) }}"><i class="fas fa-eye text-light-primary"></i> {{ __('messages.mais_detalhes') }} </a>
                                                    @endif
                                                    @if (Auth::user()->can('editar todos') || Auth::user()->can('editar tarefario'))
                                                    <a class="dropdown-item" href="{{ route('tarefarios.edit', $item->id) }}"><i class="fas fa-edit text-light-success"></i> {{ __('messages.actualizar') }}</a>
                                                    @endif
                                                    <div class="dropdown-divider"></div>
                                                    @if (Auth::user()->can('eliminar todos') || Auth::user()->can('eliminar tarefario'))
