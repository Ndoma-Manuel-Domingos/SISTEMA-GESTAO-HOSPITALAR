                    ...
                    <form action="{{ route('consultas.store') }}" method="post" class="">
                        <div class="card">
                            ...
                            <div class="card-body row">
                                ...
                                <div class="col-12 col-md-3">
                                    <label for="data_consulta" class="form-label">{{ __('messages.data') }}</label>
                                    <div class="input-group mb-3">
                                        <input type="date" class="form-control" name="data_consulta" id="data_consulta" value="" placeholder="Informe a Data da consulta">
                                    </div>
                                </div>

                                <div class="col-12 col-md-6">
                                    <label for="medico_id" class="form-label">Selecionar Médico</label>
                                    <div class="input-group mb-3">
                                        <select type="text" class="form-control select2" name="medico_id" id="medico_id">
                                            <option value="">{{ __('messages.escolher') }} </option>
                                            @foreach ($medicos as $item)
                                            <option value="{{ $item->id ?? "" }}" {{ $item->id == old('medico_id') ? 'selected' : '' }}>
                                ...
                                <div class="col-12 col-md-3">
                                    ...
                                    <div class="input-group mb-3">
                                        <select id="hora_consulta" name="hora_consulta" class="form-control">
                                ...
                                <div class="col-12 col-md-6">
                                    ...
                                    <div class="input-group mb-3">
                                        <textarea name="movito_agendamento" class="form-control" id="movito_agendamento" cols="30" rows="2" placeholder="Informe o Motivo do Agendamento"></textarea>
                                    </div>
                                </div>
                                <div class="col-12 col-md-6">
    ...
    $('#data_consulta').change(function() {
        ...
        $.ajax({
            ...
            , success: function(lista) {
                ...
                $('#medico_id').html(option);
                $('#hora_consulta').html('<option>Selecione um médico</option>');
            }
            , error: function(xhr) {
    ...
    $('#medico_id').change(function() {
        ...
        $.ajax({
            ...
            , success: function(lista) {
                let option = '';
                $.each(lista, function(i, item) {
                    option += '<option value="' + item.inicio + '">' +
                        item.inicio + ' - ' + item.fim +
                        '</option>';
                });
                $('#hora_consulta').html(option);
                Swal.close();
            }
            , error: function(xhr) {
                // Feche o alerta de carregamento
    ...
    $(document).on('click', '.delete-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: `{{ route('adicionar-items-consultas', ':id') }}`.replace(':id', recordId)
                    , method: 'DELETE'
                    , data: {
        ...
        $('form').on('submit', function(e) {
            ...
            $.ajax({
                ...
                , success: function(response) {
                    ...
                    let url = routes.atendimento.replace(':id', response.consulta.atendimento_id);
