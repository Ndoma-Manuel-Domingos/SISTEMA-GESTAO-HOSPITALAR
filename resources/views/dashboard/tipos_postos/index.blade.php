...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Tipos de Postos</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
    ...
    <form action="{{ route('tipos-postos.store') }}" method="post" class="">
        @csrf
        <div class="modal fade" id="modal-lg">
            <div class="modal-dialog modal-lg  modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">Tipo de Posto</h4>
...
<script>
    ...
    $(document).on('click', '.edit-folder', function(e) {
        ...
        $.ajax({
            url: `{{ route('tipos-postos.edit', ':id') }}`.replace(':id', recordId)
            , method: 'GET'
            , data: {
    ...
    $(document).on('click', '.delete-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                // Envia a solicitação AJAX para excluir o registro
                $.ajax({
                    url: `{{ route('tipos-postos.destroy', ':id') }}`.replace(':id', recordId)
                    , method: 'DELETE'
                    , data: {
                        _token: '{{ csrf_token() }}', // Inclui o token CSRF
                    }
