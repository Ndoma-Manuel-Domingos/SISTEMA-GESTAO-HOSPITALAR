...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('messages.conta') }}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body row py-4">
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <select type="text" class="form-control" id="classe_id" name="classe_id">
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <input type="text" class="form-control @error('serie') is-invalid @enderror" name="serie" id="serie" value="{{ old('serie') }}" placeholder="{{ __('messages.designacao') }}">
                        </div>
...
<script>
    ...
    $(document).on('click', '.edit-folder', function(e) {
        ...
        $.ajax({
            url: `{{ route('contas.edit', ':id') }}`.replace(':id', recordId)
            , method: 'GET'
            , data: {
            ...
            , success: function(response) {
                ...
                document.getElementById('conta').value = response.data.conta;
                document.getElementById('serie').value = response.data.serie;
                document.getElementById('classe_id').value = response.data.classe_id;
    ...
    $(document).on('click', '.delete-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                // Envia a solicitação AJAX para excluir o registro
                $.ajax({
                    url: `{{ route('contas.destroy', ':id') }}`.replace(':id', recordId)
                    , method: 'DELETE'
                    , data: {
                        _token: '{{ csrf_token() }}', // Inclui o token CSRF
                    }
