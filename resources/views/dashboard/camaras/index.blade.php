...
<div class="content-wrapper">
                ...
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">
                                @if (Auth::user()->can('criar todos') || Auth::user()->can('criar camara'))
                                        ...
                                        <td class="text-right">
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu" role="menu">
                                                    ...
                                                    <a class="dropdown-item" href="{{ route('camaras.show', $item->id) }}"><i class="fas fa-eye text-light-primary"></i> {{ __('messages.mais_detalhes') }} </a>
                                                    @endif

                                                    @if (Auth::user()->can('editar todos') || Auth::user()->can('editar camara'))
    ...
    <form action="{{ route('camaras.store') }}" method="post" class="">
        @csrf
                    ...
                    <div class="modal-body row py-4">
                        <div class="col-12 col-md-6">
                            ...
                            <div class="input-group mb-3">
                                <input type="text" class="form-control" name="nome" id="nome" value="{{ old('nome') }}" placeholder="Ex: Câmara 1">
                            </div>
                        </div>
                        <div class="col-12 col-md-3">
                            <label for="capacidade" class="form-label">Capacidade</label>
                            <div class="input-group mb-3">
                                <input type="text" class="form-control" name="capacidade" id="capacidade" value="{{ old('capacidade') }}" placeholder="Nº total de gavetas">
                            </div>
                        </div>
                        <div class="col-12 col-md-3">
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <div class="input-group mb-3">
                                <input type="text" class="form-control" name="localizacao" id="localizacao" value="{{ old('localizacao') }}" placeholder="Onde está instalada">
                            </div>
                        </div>
                        <div class="col-12 col-md-6">
                            <label for="temperatura_atual" class="form-label">Temperatura actual</label>
                            <div class="input-group mb-3">
                                <input type="text" class="form-control" name="temperatura_atual" id="temperatura_atual" value="{{ old('temperatura_atual') }}" placeholder="Temperatura atual">
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    $(document).on('click', '.edit-folder', function(e) {
        ...
        $.ajax({
            url: `{{ route('camaras.edit', ':id') }}`.replace(':id', recordId)
            , method: 'GET'
            , data: {
            ...
            , success: function(response) {
                ...
                document.getElementById('localizacao').value = response.data.localizacao;
                document.getElementById('temperatura_atual').value = response.data.temperatura_atual;
    ...
    $(document).on('click', '.delete-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                // Envia a solicitação AJAX para excluir o registro
                $.ajax({
                    url: `{{ route('camaras.destroy', ':id') }}`.replace(':id', recordId)
                    , method: 'DELETE'
                    , data: {
                        _token: '{{ csrf_token() }}', // Inclui o token CSRF
                    }
