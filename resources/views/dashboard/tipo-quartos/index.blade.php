...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Tipo de Quarto</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('messages.voltar') }}</a></li>
                        <li class="breadcrumb-item active">Tipos de Quartos</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    ...
    <div class="content">
                                        ...
                                        <td class="text-right">
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu" role="menu">
                                                    ...
                                                    <a class="dropdown-item" href="{{ route('tipo-quartos.show', $item->id) }}"><i class="fas fa-eye text-light-primary"></i> {{ __('messages.mais_detalhes') }} </a>
                                                    @endif
        ...
        <form action="{{ route('tipo-quartos.store') }}" method="post" class="">
            @csrf
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body row py-4">
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <div class="input-group mb-3">
                                    <input type="text" class="form-control form-control-lg" name="codigo" id="codigo" value="{{ old('codigo') }}" placeholder="Informe o código">
                                </div>
                                <p class="text-light-danger">
                                    @error('codigo')
                                    {{ $message }}
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <div class="input-group mb-3">
                                    <input type="text" class="form-control form-control-lg" name="preco" id="preco" value="{{ old('preco') ?? 0 }}" placeholder="Informe o Preço">
                                </div>
                                <p class="text-light-danger">
                                    @error('preco')
                                    {{ $message }}
...
<script>
    ...
    $(document).on('click', '.edit-folder', function(e) {
        ...
        $.ajax({
            url: `{{ route('tipo-quartos.edit', ':id') }}`.replace(':id', recordId)
            , method: 'GET'
            , data: {
    ...
    $(document).on('click', '.delete-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                // Envia a solicitação AJAX para excluir o registro
                $.ajax({
                    url: `{{ route('tipo-quartos.destroy', ':id') }}`.replace(':id', recordId)
                    , method: 'DELETE'
                    , data: {
                        _token: '{{ csrf_token() }}', // Inclui o token CSRF
                    }
