...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('messages.contratos') }}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">
                                ...
                                <a href="{{ route('contratos.create') }}" class="btn btn-light-primary"> <i class="fas fa-plus"></i> {{ __('messages.novo') }}</a>
                                @endif
                            </h3>
                        ...
                        <div class="card-body table-responsive">
                            <table class="table table-hover text-nowrap" id="carregar_tabela" style="width: 100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ $item->id ?? "" }}</td>
                                        <td><a href="{{ route('contratos.show', $item->id) }}">{{ $item->numero }}</a></td>
                                        ...
                                        <td class="text-right">
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu" role="menu">
                                                    ...
                                                    <a class="dropdown-item mudar-record" data-id="{{ $item->id ?? "" }}" href="#"><i class="fas fa-times text-light-danger"></i> {{ __('messages.desactivo') }}</a>
                                                    @else
                                                    <a class="dropdown-item mudar-record" data-id="{{ $item->id ?? "" }}" href="#"><i class="fas fa-check text-light-success"></i>{{ __('messages.activo') }}</a>
                                                    @endif
    ...
    $(document).on('click', '.mudar-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: `{{ route('recurso-humanos.mudar-estado-contratos', ':id') }}`.replace(':id', recordId)
                    , method: 'GET'
                    , data: {
    ...
    $(document).on('click', '.delete-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                // Envia a solicitação AJAX para excluir o registro
                $.ajax({
                    url: `{{ route('contratos.destroy', ':id') }}`.replace(':id', recordId)
                    , method: 'DELETE'
                    , data: {
                        _token: '{{ csrf_token() }}', // Inclui o token CSRF
                    }
