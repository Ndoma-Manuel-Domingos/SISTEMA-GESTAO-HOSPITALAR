        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-12 bg-light">
                    <div class="card">
                        <form action="{{ route('vendas_por_artigo_anterior', ['tipo' => $isMateriaPrima ? 'materias-primas' : 'produto']) }}" method="get">
                            @csrf
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-12 col-md-2">
                                        ...
                                        <select type="text" class="form-control select2" id="loja_id" name="loja_id">
                                            ...
                                            <option value="{{ $item->id ?? "" }}" {{ request()->loja_id == $item->id ? 'selected' : ''}}>{{ $item->nome }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-12 col-md-2">
                                        ...
                                        <select type="text" class="form-control select2" id="apenas_com_quantidade" name="apenas_com_quantidade">
                                            ...
                                            <option value="true">Apenas com {{ __('messages.quantidade') }}</option>
                                            <option value="false">Apenas sem {{ __('messages.quantidade') }}</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-12 col-md-2">
                                        ...
                                        <select class="form-control select2" name="categoria_id">
                                            ...
                                            <option value="{{ $categoria->id }}" {{ request()->categoria_id == $categoria->id ? 'selected' : ''}}>{{ $categoria->categoria }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-12 col-md-2">
                                        ...
                                        <div class="input-group mb-3">
                                            <input type="date" class="form-control" id="data_inicio" value="{{ request()->data_inicio ?? '' }}" name="data_inicio" placeholder="Data Inicio">
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-12 col-md-2">
                                        ...
                                        <div class="input-group mb-3">
                                            <input type="date" class="form-control" id="data_final" value="{{ request()->data_final ?? '' }}" name="data_final" placeholder="Data final">
                                        </div>
                                    </div>
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="card-tools">
                                <a href="{{ route('pdf-stock-artigos-excel', ['tipo' => $isMateriaPrima ? 'materias-primas' : 'produto', 'categoria_id' => request()->categoria_id ?? '','data_inicio' => request()->data_inicio ?? '', 'data_final' => request()->data_final ?? '', 'loja_id' => request()->loja_id ?? '', 'apenas_com_quantidade' => request()->apenas_com_quantidade ?? '']) }}" target="_blank" class="btn btn-light-success float-right"><i class="fas fa-file-excel"></i> {{ __('messages.exportar_pdf') }}</a>
                                <a href="{{ route('pdf-stock-artigos', ['tipo' => $isMateriaPrima ? 'materias-primas' : 'produto', 'categoria_id' => request()->categoria_id ?? '','data_inicio' => request()->data_inicio ?? '', 'data_final' => request()->data_final ?? '', 'loja_id' => request()->loja_id ?? '', 'apenas_com_quantidade' => request()->apenas_com_quantidade ?? '']) }}" target="_blank" class="btn btn-light-primary float-right"><i class="fas fa-file-pdf"></i> {{ __('messages.exportar_pdf') }}</a>
                            </div>
                        </div>
                            ...
                            <table class="table table-hover text-nowrap" id="carregar_tabela">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-right">Quantidade Stock</th>
                                        <th class="text-right">Total Liquido Consumida</th>
                                        <th class="text-right">Total Liquido Stock</th>
                                        @else
                                        <th class="text-right">{{ __('messages.quantidade_produtos_vendidos') }}</th>
                                        <th class="text-right">{{ __('messages.quantidade_produtos_stock') }}</th>
