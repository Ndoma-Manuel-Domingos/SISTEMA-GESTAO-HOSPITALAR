...
<div class="content-wrapper">
                ...
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">
                                @if (Auth::user()->can('criar todos') || Auth::user()->can('criar receita'))
                                        ...
                                        <td class="text-right">
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu" role="menu">
                                                    ...
                                                    <a class="dropdown-item" href="{{ route('receitas.show', $item->id) }}"><i class="fas fa-eye text-light-primary"></i> {{ __('messages.mais_detalhes') }} </a>
                                                    @endif

                                                    @if (Auth::user()->can('listar todos') || Auth::user()->can('listar receita'))
                                                    @if ($item->status == "desactivo")
                                                    <a class="dropdown-item desactivo" data-id="{{ $item->id ?? "" }}" href="#"><i class="fas fa-check text-light-primary"></i> {{ __('messages.activo') }}</a>
                                                    @endif

                                                    @if ($item->status == "activo")
                                                    <a class="dropdown-item activo" data-id="{{ $item->id ?? "" }}" href="#"><i class="fas fa-times text-light-primary"></i> {{ __('messages.desactivo') }}</a>
                                                    @endif
                                                    @endif

                                                    @if (Auth::user()->can('editar todos') || Auth::user()->can('editar receita'))
                                                    ...
                                                    @if (Auth::user()->can('eliminar todos') || Auth::user()->can('eliminar receita'))
    ...
    <form action="{{ route('receitas.store') }}" method="post" class="">
        @csrf
        <div class="modal fade" id="modal-lg">
            <div class="modal-dialog modal-lg  modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">{{ __('messages.receita') }}</h4>
...
<script>
    ...
    $(document).on('click', '.edit-folder', function(e) {
        ...
        $.ajax({
            url: `{{ route('receitas.edit', ':id') }}`.replace(':id', recordId)
            , method: 'GET'
            , data: {
    ...
    $(document).on('click', '.delete-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                // Envia a solicitação AJAX para excluir o registro
                $.ajax({
                    url: `{{ route('receitas.destroy', ':id') }}`.replace(':id', recordId)
                    , method: 'DELETE'
                    , data: {
                        _token: '{{ csrf_token() }}', // Inclui o token CSRF
                    }
    ...
    $(document).on('click', '.desactivo', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                // Envia a solicitação AJAX para excluir o registro
                $.ajax({
                    url: `{{ route('activar-receitas', ':id') }}`.replace(':id', recordId)
                    , method: 'GET'
                    , data: {
                        _token: '{{ csrf_token() }}', // Inclui o token CSRF
                    }
    ...
    $(document).on('click', '.activo', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                // Envia a solicitação AJAX para excluir o registro
                $.ajax({
                    url: `{{ route('desactivar-receitas', ':id') }}`.replace(':id', recordId)
                    , method: 'GET'
                    , data: {
                        _token: '{{ csrf_token() }}', // Inclui o token CSRF
                    }
