    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="text-align: left;padding: 3px">Reembolso</td>
                @else
                <td style="text-align: left;padding: 3px">Venda</td>
                @endif
                <td style="text-align: left;padding: 3px">{{ $item->caixa->conta ?? '' }} {{ $item->caixa->nome ?? '' }}</td>
                <td style="text-align: left;padding: 3px">{{ $item->user->name ?? '' }}</td>

                @if ($item->factura->pagamento == 'NU')
                ...
                @php $debito += $item->valor_pagar; @endphp
                @else
                @php $credito += $item->valor_pagar; @endphp
                <td style="text-align: left;color: red;padding: 3px">Saída</td>
                @endif

                @if ($item->status === "realizado")
                <td style="text-align: right;color: blue;padding: 3px"> {{ number_format($item->valor_pagar ?? 0, 2, ',', '.') }}</td>
                @else
                <td style="text-align: right;color: red;padding: 3px"> {{ number_format($item->valor_pagar ?? 0, 2, ',', '.') }}</td>
                @endif
