    ...
    <main style="position: absolute;top: 240px;right: 30px;left: 30px;">
        <table>
            <thead>
                ...
                <tr>
                    <th colspan="4" style="border: 1px solid #010101;text-align: left;padding: 3px"> {{ $requests['status'] ?? 'TODAS' }}</th>
                </tr>
            </thead>
        </table>
        ...
        <table>
            <thead>
                <tr>
                    <th style="border: 1px solid #010101;padding: 2px;">Nº</th>
                    <th style="border: 1px solid #010101;padding: 2px;">Solicitação Nº</th>
                    <th style="border: 1px solid #010101;padding: 2px;">Paciente</th>
                    <th style="border: 1px solid #010101;padding: 2px;">{{ __('messages.estados') }}</th>
                    <th style="border: 1px solid #010101;padding: 2px;">Prioridade</th>
                    <th style="border: 1px solid #010101;padding: 2px;"> {{ __('messages.data') }} </th>
                </tr>
            </thead>
            @foreach ($solicitacoes as $key => $item)
            <tr>
                <td style="text-align: left;padding: 3px">{{ $key + 1 }}</td>
                <td style="text-align: left;padding: 3px">{{ $item->solicitacao }}</td>
                <td style="text-align: left;padding: 3px">{{ $item->paciente->nome }}</td>
                <td style="text-align: left;padding: 3px">{{ $item->status }}</td>
                <td style="text-align: left;padding: 3px">{{ $item->prioridade->nome }}</td>
                <td style="text-align: left;padding: 3px">{{ $item->created_at }}</td>
            </tr>
            @endforeach
            <tfoot>
                <tr>
                    <th colspan="3" style="border: 1px solid #010101;text-align: left;padding: 3px">Total Registros</th>
                    <th colspan="3" style="border: 1px solid #010101;text-align: right;padding: 3px">{{ count($solicitacoes) }}</th>
                </tr>
            </tfoot>
        </table>
