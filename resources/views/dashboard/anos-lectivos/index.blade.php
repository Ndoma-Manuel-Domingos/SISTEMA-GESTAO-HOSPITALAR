                ...
                <div class="col-12 col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">
                                <a href="{{ route('anos-lectivos.create') }}" class="btn btn-light-primary"> <i class="fas fa-plus"></i> {{ __('messages.novo') }}</a>
                            </h3>
                        </div>
                                        ...
                                        <td class="text-right">
                                            <div class="btn-group">
                                                ...
                                                <div class="dropdown-menu" role="menu">
                                                    <a class="dropdown-item" href="{{ route('anos-lectivos.show', $item->id) }}"><i class="fas fa-eye text-light-primary"></i> {{ __('messages.mais_detalhes') }} </a>
                                                    <a class="dropdown-item" href="{{ route('anos-lectivos.edit', $item->id) }}"><i class="fas fa-edit text-light-success"></i> {{ __('messages.actualizar') }}</a>
                                                    @if ($item->status == 'activo')
                                                    <a class="dropdown-item mudar-status-record" data-id="{{ $item->id ?? "" }}"><i class="fas fa-times text-light-danger"></i> {{ __('messages.desactivo') }}</a>
                                                    @else
                                                    <a class="dropdown-item mudar-status-record" data-id="{{ $item->id ?? "" }}"><i class="fas fa-check text-light-success"></i> {{ __('messages.activo') }}</a>
                                                    @endif
    ...
    $(document).on('click', '.mudar-status-record', function(e) {
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: `{{ route('anos-lectivos.mudar-status', ':id') }}`.replace(':id', recordId)
                    , method: 'GET'
                    , data: {
