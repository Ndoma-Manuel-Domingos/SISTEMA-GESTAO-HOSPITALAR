...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{ __('messages.mesa') }}: {{ $mesa ? $mesa->nome : '' }}</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('mesas.visualizacao-mesas') }}" class="btn btn-light-primary">{{ __('messages.voltar') }}</a></li>
                        <li class="breadcrumb-item"><a href="#" data-id="{{ $mesa->id }}" class="btn enviar_pedidos_a_cuzinha btn-light-primary">ENVIAR PARA CUZINHA</a></li>
                        <li class="breadcrumb-item"><a href="#" data-id="{{ $mesa->id }}" class="btn desocupar_mesas btn-light-danger">DESOCUPAR A MESA</a></li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                                        ...
                                        <div class="tab-content" id="custom-tabs-four-tabContent">
                                            <div class="tab-pane fade show active" id="custom-tabs-four-home" role="tabpanel" aria-labelledby="custom-tabs-four-home-tab">
                                                <div class="row" id="carregar_produtos">
                                                    ...
                                                    <div class="col-6 col-md-3 col-lg-2">
                                                        <a class="adicionar-carrinho" style="cursor: pointer" data-stock="{{ $item->total_produto_loja_activa() }}" data-mesa="{{ $mesa->id }}" data-id="{{ $item->id ?? "" }}" data-nome="{{ $item->nome }}" data-preco="{{ $item->preco_venda_com_iva }}">
                                            ...
                                            <div class="tab-pane fade" id="custom-tabs-four-profile{{ $categoria->id }}" role="tabpanel" aria-labelledby="custom-tabs-four-profile-tab{{ $categoria->id }}">
                                                <div class="row">
                                                    ...
                                                    <div class="col-6 col-md-3 col-lg-2">
                                                        <a class="adicionar-carrinho" style="cursor: pointer" data-stock="{{ $produto->total_produto_loja_activa() }}" data-mesa="{{ $mesa->id }}" data-id="{{ $produto->id }}" data-nome="{{ $produto->nome }}" data-preco="{{ $produto->preco_venda_com_iva }}">
                                                            <div class="card shadow-sm bg-light">
                    ...
                    <div class="card text-center">
                        ...
                        <div class="card-body text-center">
                            ...
                            <form action="{{ route('caixa.abertura_caixa_create') }}" method="post" class="px-5">
                                ...
                                <button type="submit" class="btn mt-4 btn-md d-iniline-block btn-light-primary"><i class="fas fa-box"></i> {{ __('messages.activar_caixa') }}</button>
                            </form>
                            ...
                            <div class=" card-body" style="height: auto;">
                                <div action="" class="row">
                                    ...
                                    <div class="col-12 col-md-1">
                                        ...
                                        <label style="font-size: 1rem; display: flex; align-items: center; gap: 10px; cursor: pointer;">
                                            <input type="checkbox" name="checkinBox" id="checkinBox" style="width: 130px; height: 130px;">
                                            Usar o Cartão comsumo
                                        </label>
                                    ...
                                    <div class="col-md-8 col-12">
                                        <div class="row">
                                            ...
                                            <div class="col-md-6 col-12 mb-4">
                                                <div class="row">
                                                    <div class="col-md-6 col-12 mb-4">
                                                        <input class="form-control form-control-lg" title="ESCOLHER O NOME DO CLIENTE" type="text" name="nome_cliente" id="nome_cliente" value="CONSUMIDOR FINAL">
                                                    </div>

                                                    <div class="col-md-6 col-12 mb-4">
                                                        <input class="form-control form-control-lg" title="ESCOLHER O NOME DO NIF DO CLIENTE" type="text" name="documento_nif" id="documento_nif" value="999999999">
                                                    </div>
                                    ...
                                    <input type="hidden" name="mesa_id" class="mesa_id" id="mesa_id" value="{{ $mesa ? $mesa->id : '' }}">
                                    <input type="hidden" id="venda_realizado" name="venda_realizado" value="MESA">
                    ...
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            ...
                            <div class="modal-header">
                                <h5 class="modal-title" id="modalLabel">{{ __('messages.quantidade') }}</h5>
...
<script>
    ...
    $(document).on('click', '.enviar_pedidos_a_cuzinha', function(e) {
        ...
        Swal.fire({
            ...
            , text: "Desejo enviar os pedidos a cuzinha!"
            , icon: 'warning'
        ...
        , }).then((result) => {
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: `{{ route('enviar-pedidos.cuzinha', ':id') }}`.replace(':id', recordId)
                    , method: 'GET'
                    , data: {
    ...
    $(document).on('click', '.desocupar_mesas', function(e) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function carregarCarrinho() {
            fetch(`../../carrinho/carregar-pedidos-mesa?mesa=${document.getElementById("mesa_id").value}`, {
                    method: 'GET'
                    , headers: {
                        'Content-Type': 'application/json'
        ...
        document.querySelectorAll('.adicionar-carrinho').forEach(button => {
            button.addEventListener('click', function() {
                ...
                adicionarAoCarrinho(produtoId, nome, preco, mesa, quantidade);
        ...
        // Função para adicionar um produto ao carrinho
        function adicionarAoCarrinho(produtoId, nome, preco, mesa, quantidade = 1) {
            fetch('../../carrinho/adicionar-mesa', {
                    method: 'POST'
                    , headers: {
                        'Content-Type': 'application/json'
                        , 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute(
                    ...
                    , success: function(response) {
                        ...
                        for (let index = 0; index < response.produtos
                            ...
                            $('#carregar_produtos').append('<div class="col-6 col-md-3 col-lg-2">\
                                    <a id="adicionar-carrinho-exemplo" style="cursor: pointer;" data-mesa="' + mesa +
                                '" data-id="' + response.produtos[index].id +
                                '" data-nome="' + response.produtos[index]
                                .nome + '" data-preco="' + response.produtos[
                                    index].preco_venda + '">\
                                        <div class="card shadow-sm bg-light">\
                                ...
                                .nome + '</h6>\
                                                    <p class="text-white"><strong>' + response.produtos[index]
                                .preco_venda + '<small>' + response.loja.moeda + '</small></strong></p>\
                                                </div>\
                                            </div>\
                                        </div>\
                                    </a>\
