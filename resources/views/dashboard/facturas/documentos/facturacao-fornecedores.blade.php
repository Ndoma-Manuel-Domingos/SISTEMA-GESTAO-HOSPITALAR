...
<body>
    ...
    <table class="table">
        ...
        <tbody>
            @php
            $valor_factura = $valor_divida = $total_pago = 0;
            @endphp
            @if ($facturas)
            ...
            <tr>
                ...
                <td style="padding: 3px;text-align: left">{{ $item->data_factura }}</td>
                <td style="padding: 3px;text-align: left">{{ $item->data_vencimento }}</td>
                <td style="padding: 3px;text-align: left"><span class="bg-light-warning p-1"><i class="fas fa-exclamation-triangle"></i></span> {{ $item->status == true ? 'Pago' : 'Não Pago' }}</td>
                <td style="padding: 3px;text-align: right">{{ number_format($item->valor_factura, 2, ',', '.') }}
                <td style="padding: 3px;text-align: right">{{ number_format($item->valor_divida, 2, ',', '.') }}
                <td style="padding: 3px;text-align: right">{{ number_format($item->total_pago, 2, ',', '.') }} {{ $empresa_logada->empresa->moeda }}</td>
            </tr>

            @php
            $valor_factura += $item->valor_factura; $valor_divida += $item->valor_divida; $total_pago += $item->total_pago;
            @endphp
        ...
        <tfoot>
            <tr>
                <th colspan="7" style="border: 1px solid #010101;padding: 2px;text-align: right"></th>
                <th colspan="" style="border: 1px solid #010101;padding: 2px;text-align: right">{{ number_format($valor_divida, 2, ',', '.') }}</th>
                <th style="border: 1px solid #010101;padding: 2px;text-align: right">{{ number_format($total_pago, 2, ',', '.') }}</th>
            </tr>
            <tr>
